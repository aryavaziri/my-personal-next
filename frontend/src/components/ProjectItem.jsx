...
export const ProjectItem = ({ item, setHoveredItem, edit }) => {
  ...
  const [delProject, { data, loading, error: delErr, reset }] = useMutation(MUTATION);
  ...
  return (
    ...
    (deleted) ? <p className={`duration-[3000] text-rose-400 fade-out`}>Deleted successfully</p> :
      loading ? <p>Deleting...</p>
        : <>
          <OK active={errMode} setActive={setErrMode} message={delErr?.message} />
...
export const ProjectMedia = ({ item }) => {
  ...
  const src = `${(process.env.NODE_ENV == 'development') ? 'https://aryav.nl' : ''}/static/projects/${item._id}.${item.extention}`
  useEffect(() => {
  ...
  return (item.extention.toLowerCase() === "mov" || item.extention.toLowerCase() === "mp4" || item.extention.toLowerCase() === "mkv")
    ? (
      <video
        ...
        playsInline
      >
        <source src={src} alt={item.title} type="video/mp4" />
      </video>
